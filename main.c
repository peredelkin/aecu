...
coil_act_t coil14on = make_coil_act(coil14_off, 0);
coil_act_t coil14off = make_coil_act(coil14_on, 114);
coil_act_t coil23on = make_coil_act(coil23_on, 180);
coil_act_t coil23off = make_coil_act(coil23_off, 294);
...
void main_handler() {
    coil_act_handler(&coil_buffer,&root,&t4ch1,&tcnt4_mask,capture,angle_counter);
    angle_counter += 6;
}
